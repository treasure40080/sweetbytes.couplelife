...
private val PRESET_CATEGORIES_EDIT = listOf("吃飯", "玩樂", "薪水", "租屋費", "其他")

@Composable
fun EditEntryDialog(
    entry: EntryEntity,
    onConfirm: (EntryEntity) -> Unit,
    onDismiss: () -> Unit
) {
    val isPreset = PRESET_CATEGORIES_EDIT.dropLast(1).contains(entry.category)
    var selectedTypeIndex by remember {
        mutableStateOf(if (entry.type == EntryType.INCOME) 0 else 1)
    }
    var amountText by remember { mutableStateOf(entry.amount.toString()) }
    var selectedCategory by remember {
        mutableStateOf(if (isPreset) entry.category else "其他")
    }
    var customName by remember {
        mutableStateOf(if (isPreset) "" else entry.category)
    }
        ...
        text = {
            Column(modifier = Modifier.fillMaxWidth()) {
                ...
                Column(modifier = Modifier.selectableGroup()) {
                    PRESET_CATEGORIES_EDIT.forEach { category ->
                        Row(
                            modifier = Modifier
                                .fillMaxWidth()
                                .selectable(
